...
def load_coco_json(json_file, image_root, metadata, dataset_name, root_dir):
    is_shots = "shot" in dataset_name  # few-shot
    if is_shots:
        imgid2info = {}
        shot = dataset_name.split('_')[-2].split('shot')[0]
        seed = int(dataset_name.split('_seed')[-1])
        split_dir = os.path.join(root_dir, 'cocosplit', 'seed{}'.format(seed))
...
def register_meta_coco(name, metadata, imgdir, annofile, root_dir):

    DatasetCatalog.register(
        name,
        lambda: load_coco_json(annofile, imgdir, metadata, name, root_dir),
    )
