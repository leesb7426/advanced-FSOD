...
def load_filtered_voc_instances(
    name: str, dirname: str, split: str, classnames: str, root_dir: str
):
    """
    ...
    if is_shots:
        fileids = {}
        split_dir = os.path.join(root_dir, "vocsplit")
    ...
    if is_shots:
        for cls, fileids_ in fileids.items():
            ...
            for fileid in fileids_:
                ...
                dirname = os.path.join(root_dir, "VOC{}".format(year))
            ...
            dicts.extend(dicts_)
            # if len(dicts_) > int(shot):
            #     dicts_temp = np.random.choice(dicts_, int(shot), replace=False)
            #     file_name_list = [d['file_name'] for d in dicts_temp]
            #     file_name_list = np.unique(file_name_list)
            #     dict_for_oracle = []
            #     for d in dicts_:
            #         if d['file_name'] in file_name_list:
            #             dict_for_oracle.append(d)
            #     dicts.extend(dict_for_oracle)
            # else:
            #     dicts.extend(dicts_)
    else:
...
def register_meta_voc(
    name, metadata, dirname, split, year, keepclasses, sid, root_dir
):
    ...
    DatasetCatalog.register(
        ...
        lambda: load_filtered_voc_instances(
            name, dirname, split, thing_classes, root_dir
        ),
    )
